...
class Surat_keluar_adp extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('surat_keluar_adp_model');
    ...
    public function index()
    {
        ...
        $config['base_url'] = base_url() . 'surat_keluar_adp/index/';
        $config['total_rows'] = $this->surat_keluar_adp_model->total_rows();
        $config['per_page'] = 10;
        $config['uri_segment'] = 3;
        $config['suffix'] = '.html';
        $config['first_url'] = base_url() . 'surat_keluar_adp.html';
        $this->pagination->initialize($config);

        $start = $this->uri->segment(3, 0);
        $surat_keluar_adp = $this->surat_keluar_adp_model->index_limit($config['per_page'], $start);

        $data = array(
            'surat_keluar_adp_data' => $surat_keluar_adp,
            'keyword' => $keyword,
        ...
        $this->load->view('surat_keluar_adp_list', $data);
    ...
    public function search() 
    {
        ...
        if ($this->uri->segment(2)=='search') {
            $config['base_url'] = base_url() . 'surat_keluar_adp/search/' . $keyword;
        } else {
            $config['base_url'] = base_url() . 'surat_keluar_adp/index/';
        }

        $config['total_rows'] = $this->surat_keluar_adp_model->search_total_rows($keyword);
        $config['per_page'] = 10;
        $config['uri_segment'] = 4;
        $config['suffix'] = '.html';
        $config['first_url'] = base_url() . 'surat_keluar_adp/search/'.$keyword.'.html';
        $this->pagination->initialize($config);

        $start = $this->uri->segment(4, 0);
        $surat_keluar_adp = $this->surat_keluar_adp_model->search_index_limit($config['per_page'], $start, $keyword);
    ...
    {
        $row = $this->surat_keluar_adp_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'dokumen2' => $row->dokumen2,
			...
			$this->load->view('backend/_template/header');
			$this->load->view('backend/_template/sidebar');
            $this->load->view('surat_keluar_adp_read', $data);
			$this->load->view('backend/_template/footer');
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('surat_keluar_adp'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('surat_keluar_adp/create_action'),
		...
		$this->load->view('backend/_template/header');
        $this->load->view('backend/_template/sidebar');
        $this->load->view('surat_keluar_adp_form', $data);
		$this->load->view('backend/_template/footer');
    }
	...
	public function create_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $config['upload_path'] = "./uploads/surat_keluar_adp/";
			...
			if( !$this->upload->do_upload('dokumen2')){
				...
				$this->surat_keluar_adp_model->insert($data);
	...
	public function diajukan_diteruskan($id){
		...
		$this->load->view('backend/_template/header');
        $this->load->view('backend/_template/sidebar');
        $this->load->view('surat_keluar_adp_diajukan_form', $data);
		$this->load->view('backend/_template/footer');
		}else{
			$this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('surat_keluar_adp'));
	...
	public function diajukan_diteruskan_create_action(){
		...
		$this->session->set_flashdata('message', 'Create Record Success');
		redirect(site_url('surat_diajukan/surat_keluar_adp'));
    }
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('surat_keluar_adp/update_action'),
    ...
    $this->_rules();
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			if(!empty($_FILES['dokumen']['name'])){
				...
				$config['encrypt_name']  = TRUE;
				unlink('./uploads/surat_masuk_adp/'.$row);
				$this->upload->initialize($config); //meng set config yang sudah di atur
				...
				$this->surat_keluar_adp_model->update($this->input->post('id_surat', TRUE), $data);
	...
	public function delete($id) 
    {
        ...
        if ($row) {
			...
			//echo $gambar;
			unlink('./uploads/surat_keluar_adp/'.$dokumen);
            $this->surat_keluar_adp_model->delete($id);
            $this->surat_diajukan_model->delete_all_surat_keluar_adp($id);
    ...
    public function excel()
    {
        ...
        $namaFile = "surat_keluar_adp.xls";
        $judul = "surat_keluar_adp";
        $tablehead = 2;
        $tablebody = 3;
        $nourut = 1;
        //penulisan header
	...
	foreach ($this->surat_keluar_adp_model->get_all() as $data) {
            $kolombody = 0;
    ...
    public function word()
    {
        ...
        header("Content-Disposition: attachment;Filename=surat_keluar_adp.doc");

        $data = array(
            'surat_keluar_adp_data' => $this->surat_keluar_adp_model->get_all(),
            'start' => 0
        );
        
        $this->load->view('surat_keluar_adp_html',$data);
    }

};

/* End of file surat_keluar_adp.php */
/* Location: ./application/controllers/surat_keluar_adp.php */